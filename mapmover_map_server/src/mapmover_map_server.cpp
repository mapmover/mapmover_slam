...
#include "mapmover_marker_drawing/MapmoverDrawings.h"
#include "mapmover_map_tools/MapmoverMapTools.h"

#include "mapmover_nav_msgs/GetDistanceToObstacle.h"
#include "mapmover_nav_msgs/GetSearchPosition.h"


class OccupancyGridContainer
{
public:
  OccupancyGridContainer(std::string sub_topic, std::string prefix, ros::NodeHandle& nh, MapmoverDrawings* drawing_provider, tf::TransformListener* tf_)
  ...
  {
    ROS_INFO("mapmover_map_server map service called");
  ...
  bool lookupServiceCallback(mapmover_nav_msgs::GetDistanceToObstacle::Request  &req,
                          mapmover_nav_msgs::GetDistanceToObstacle::Response &res )
  {
    //ROS_INFO("mapmover_map_server lookup service called");
  ...
  bool getSearchPosServiceCallback(mapmover_nav_msgs::GetSearchPosition::Request  &req,
                                   mapmover_nav_msgs::GetSearchPosition::Response &res )
  {
    if (!map_ptr_){
...
/*
  ...
  MapmoverMapTools::DistanceMeasurementProvider dist_meas_;

  MapmoverDrawings* drawing_provider_;
...
class MapmoverMapServer
{
  public:
    /** Trivial constructor */
    MapmoverMapServer(ros::NodeHandle& private_nh)
    {
      std::string frame_id;

      mapmover_drawings_ = new MapmoverDrawings();
      mapmover_drawings_->setNamespace("map_server");

      mapContainer = new OccupancyGridContainer("map", "" ,private_nh, mapmover_drawings_,&tf_);
    }
...
private:
    ...
    MapmoverDrawings* mapmover_drawings_;
    tf::TransformListener tf_;
};

int main(int argc, char **argv)
{
  ros::init(argc, argv, "mapmover_map_server");
  ros::NodeHandle nh;

  MapmoverMapServer ms(nh);
