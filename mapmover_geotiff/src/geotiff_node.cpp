...
public:
    ...
    , running_saved_map_num_(0)
  {
    ...
    path_service_client_ = n_.serviceClient<mapmover_nav_msgs::GetRobotTrajectory>("trajectory");
    ...
    if ((plugin_list.size() > 0) && (plugin_list[0].length() > 0)){
      plugin_loader_ = std::make_unique<pluginlib::ClassLoader<mapmover_geotiff::MapWriterPluginInterface>>("mapmover_geotiff", "mapmover_geotiff::MapWriterPluginInterface");

      for (size_t i = 0; i < plugin_list.size(); ++i){
        try
        {
          boost::shared_ptr<mapmover_geotiff::MapWriterPluginInterface> tmp (plugin_loader_->createInstance(plugin_list[i]));
  ...
  std::unique_ptr<pluginlib::ClassLoader<mapmover_geotiff::MapWriterPluginInterface>> plugin_loader_;
  std::vector<boost::shared_ptr<mapmover_geotiff::MapWriterPluginInterface> > plugin_vector_;
...
int main(int argc, char** argv)
{
  ...
  mapmover_geotiff::MapGenerator mg;
